...
class _LocationInputState extends State<LocationInput> {
  ...
  void _savePlace(double lat, double lng) async {
    final url = Uri.parse(
        'https://maps.googleapis.com/maps/api/geocode/json?latlng=$lat,$lng&key=$key&enable_address_descriptor=true');
  ...
  Widget build(BuildContext context) {
    Widget preiewContent;
    if (_isGettingLocation) {
      preiewContent = const CircularProgressIndicator();
    } else if (_pickedLocation != null) {
      preiewContent = Image.network(
        locationImage,
        fit: BoxFit.cover,
